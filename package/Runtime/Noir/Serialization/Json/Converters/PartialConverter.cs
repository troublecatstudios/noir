...
namespace Noir.Serialization.Json.Converters {
    public abstract class PartialConverter<T> : JsonConverter  where T : new() {
        ...
        private T InternalReadJson(JsonReader reader, JsonSerializer serializer, [AllowNull] object existingValue)
        {
            ...
            {
                throw NoirSerializationException.CreateSerializationException(reader, $"Failed to read type '{typeof(T).Name}'. Expected object start, got '{reader.TokenType}' <{reader.Value}>");
            }
            ...
            while (reader.TokenType == JsonToken.PropertyName)
            {
                if (reader.Value is string name)
                {
                    ...
                    {
                        throw NoirSerializationException.CreateSerializationException(reader, $"Failed to read type '{typeof(T).Name}'. Possible loop when reading property '{name}'");
                    }
